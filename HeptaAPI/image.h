...
public:
	...
	//파일로부터 이미지 초기화
	HRESULT init(const char* fileName, int width, int height, BOOL isAlpha = FALSE,
		BOOL trans = FALSE, COLORREF transColor = FALSE);
	//파일로부터 이미지 초기화           처음 시작 좌표      가로       세로
	HRESULT init(const char* fileName, float x, float y, int width, int height, BOOL isAlpha = FALSE,
		BOOL trans = FALSE, COLORREF transColor = FALSE);

	//이미지 + 프레임초기화
	HRESULT init(const char* fileName, float x, float y, int width, int height,
		int frameX, int frameY, BOOL isAlpha = FALSE, BOOL trans = FALSE, COLORREF transColor = RGB(255, 0, 255));

	HRESULT init(const char* fileName, int width, int height, int frameX, int frameY, BOOL isAlpha = FALSE,
	...
	// 편의상 추가한 함수
	void centerRender(HDC hdc, float x, float y, int width, int height);
};
