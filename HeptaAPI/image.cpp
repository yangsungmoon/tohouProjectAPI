...
//파일로부터 이미지 초기화
HRESULT image::init(const char* fileName, int width, int height, BOOL isAlpha,
...
HRESULT image::init(const char* fileName, float x, float y, int width, int height, BOOL isAlpha,
	BOOL trans, COLORREF transColor)
{
	//이미지 정보가 뭔가있다면 해제해줘라
	if (_imageInfo != NULL) release();
...
HRESULT image::init(const char* fileName, float x, float y, int width, int height,
	int frameX, int frameY, BOOL isAlpha, BOOL trans, COLORREF transColor)
{
	//이미지 정보가 뭔가있다면 해제해줘라
	if (_imageInfo != NULL) release();
...
void image::alphaRender(HDC hdc, int destX, int destY, int sourX, int sourY, int sourWidth, int sourHeight, BYTE alpha)
{
	...
	{
		AlphaBlend(hdc, destX, destY, sourWidth,
			sourHeight, _blendImage->hMemDC, 0, 0, sourWidth, sourHeight, _blendFunc);
	}
}
...
void image::alphaFrameRender(HDC hdc, int destX, int destY, BYTE alpha)
{
	...
	if (_trans)
	{
		...
		AlphaBlend(hdc, destX, destY, _imageInfo->frameWidth,
			_imageInfo->frameHeight, _blendImage->hMemDC, destX,
			destY, _imageInfo->frameWidth, _imageInfo->frameHeight, _blendFunc);
	}
	else
	{
		AlphaBlend(hdc, destX, destY, _imageInfo->frameWidth,
...
void image::centerRender(HDC hdc, float x, float y, int width, int height) {
	render(hdc, x - (width / 2), y - (height / 2));
	//alphaRender(hdc, x - (width / 2), y - (height / 2), 0);
}